...
//        System.out.println(t);
    ...
    public Long decipher(String l){
        if (l == null){
            throw new NullPointerException("String ingresado es nulo");
        }
        byte[] bytes = new byte[0];
        try {
            bytes = Base58.decode(l);
        } catch (AddressFormatException e) {
            e.printStackTrace();
        }
        byte[] decipher1 = decrypt(bytes);
        String s = new String(decipher1);
        return new Long(s);
    }


    public String decipher(String l, Boolean b){
        if (l == null){
            throw new NullPointerException("String ingresado es nulo");
        }
        byte[] bytes = new byte[0];
        ...
        byte[] decipher1 = decrypt(bytes);
        return new String(decipher1);
    }

    public String encipher(Long l){
        byte[] n = new byte[0];
        try {
            n = l.toString().getBytes("UTF8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        byte[] encipher1 = encrypt(n);
        return new String(Base58.encode(encipher1));
    }


    public String encipher(Integer l){
        byte[] n = new byte[0];
        try {
            n = l.toString().getBytes("UTF8");
        } catch (UnsupportedEncodingException e) {
    ...
    public String encipher(String l){
        byte[] n = new byte[0];
        try {
            n = l.toString().getBytes("UTF8");
        } catch (UnsupportedEncodingException e) {
