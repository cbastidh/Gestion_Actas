...
public class GestorDocumentalEJBRemoteBean implements GestorDocumentalEJBRemote {
    private static Logger logger = Logger.getLogger(GestorDocumentalEJBRemoteBean.class);
     ...
     */
    @Override
    public String actualizarDocumento(String username, String fileName, byte[] fileStream, DocumentoVO documentoVO, String originalDocumentName, String originalDocumentExtension) {
        DocumentoBO parentDocumentBO = documentoDAO.get(documentoVO.getParentDocument().getId());
        DocumentoBO lastDocumentVersionBO = documentoDAO.getLastVersionForParentId((documentoVO.getParentDocument().getId()));
        String documentId;
        if(fileStream != null || fileName != null){
            SaveDocument saveDocument = new SaveDocumentIDC();
            documentId = saveDocument.guardarEnUCM(username, fileName, fileStream, documentoVO.getDescripcion());
        } else {
            documentId = parentDocumentBO.getTokenDocumento();
            originalDocumentExtension = lastDocumentVersionBO.getOriginalDocumentExtension();
            originalDocumentName = lastDocumentVersionBO.getOriginalDocumentBaseName();
        }

        DocumentoBO newUpdatedDocumentoBO = new DocumentoBO(documentoVO.getTipoCorporacion(), documentoVO.getTipoDocumento(), parentDocumentBO);
        ...
        newUpdatedDocumentoBO.setOriginalDocumentExtension(originalDocumentExtension);
    ...
    /**
     * @see cl.bice.gestionactas.ejb.svc.GestorDocumentalSvc#obtenerListadoVersiones(cl.bice.gestionactas.ejb.vo.UserVO, cl.bice.gestionactas.ejb.vo.DocumentoVO)
     */
    @Override
    public List<DocumentoVO> obtenerListadoVersiones(UserVO user, DocumentoVO document) {
        List<DocumentoVO> resultList = new ArrayList<>();
        for(DocumentoBO docBO : documentoDAO.obtenerListadoVersiones(document.getId(), user.getId())){
    ...
    public PageVO<DocumentoVO> obtenerDocumentos(PageQueryMapVO query){
        ...
        List<DocumentoBO> documentoBOs = new ArrayList<>();
        Map<String, Object> mapWhere = query.getQuery();

        if (mapWhere == null){
            throw new NullPointerException("Se esperaba que el mapa de objectos para el where viniera con datos");
        }
        ...
        if(periodoFinal != null && periodoInicial != null){
            cantidad = documentoDAO.countObtenerDocumentosPorPeriodo(true, titulo, descripcion, tipoDocumentoId,
                    tipoCorporacionId, periodoInicial, periodoFinal, usuarioId);
            if (cantidad > 0) {
                List<Long> objs = documentoDAO.obtenerDocumentosPorPeriodo(true, titulo, descripcion, tipoDocumentoId,
                        tipoCorporacionId, periodoInicial, periodoFinal, usuarioId, query.getFirst(), query.getSize());
                for(Long o : objs){
            ...
            if (anno == null){
                cantidad = documentoDAO.countFiltrarDocumentosSinAnno(true, titulo, descripcion, tipoDocumentoId, tipoCorporacionId, usuarioId, sesion);
                if (cantidad > 0) {
                    /*
                	List<Long> objs = documentoDAO.filtrarDocumentosSinAnno(true, titulo, descripcion, tipoDocumentoId,
                            tipoCorporacionId, sesion, usuarioId, query.getFirst(), query.getSize());
                    for(Long o : objs){
                        targetIdList.add(o.longValue());
                    }
                    */

                	List<BigDecimal> objs = filtrarDocumentosID(titulo, descripcion, tipoDocumentoId,
            					tipoCorporacionId, sesion, anno, usuarioId, query.getFirst(), query.getSize());
				...
				//Added code
				cantidad = countFiltrarDocumentosSesionID(titulo, descripcion, tipoDocumentoId, tipoCorporacionId, sesion, anno, usuarioId);
				/*
				if(sesion != null){
					//cantidad = documentoDAO.countFiltrarDocumentosSesion(true, titulo, descripcion, tipoDocumentoId, tipoCorporacionId, usuarioId, sesion, anno);
					cantidad = countFiltrarDocumentosSesionID(titulo, descripcion, tipoDocumentoId, tipoCorporacionId, sesion, anno, usuarioId);
				} else{
					cantidad = documentoDAO.countFiltrarDocumentos(true, titulo, descripcion, tipoDocumentoId, tipoCorporacionId, usuarioId, sesion, anno);
				}
				*/
                ...
                //cantidad = documentoDAO.countFiltrarDocumentos(true, titulo, descripcion, tipoDocumentoId, tipoCorporacionId, usuarioId, sesion, anno);
                if (cantidad > 0) {
                    //List<Long> objs = documentoDAO.filtrarDocumentos(true, titulo, descripcion, tipoDocumentoId,
                    //        tipoCorporacionId, sesion, anno, usuarioId, query.getFirst(), query.getSize());
                    //for(Long o : objs){
                    //    targetIdList.add(o.longValue());
                	List<BigDecimal> objs = filtrarDocumentosID(titulo, descripcion, tipoDocumentoId,
        ...
        //if(targetdocList!= null && !targetdocList.isEmpty()) {
        if(targetIdList!= null && !targetIdList.isEmpty()) {
            //List<Object> maxVersions = documentoDAO.getLastVersionListMaxVersion(targetIdList);
            //for (Object o : maxVersions) {
            //for (Long id : targetIdList) {
                //Object[] l = (Object[]) o;
                //Long id = (Long) l[0];
                //Integer version = (Integer) l[1];
                //DocumentoVO d = documentoDAO.obtenerDocumentoByParentMaxVersion(id, version).toVO();
        	List<DocumentoBO> documents = documentoDAO.obtenerDocByIdsFecCreaDesc(targetIdList);
        	for(DocumentoBO doc : documents){
                ...
                if (isFull && sesion != null){
                    if (d.getNumSesion().equals(sesion) && c.get(Calendar.YEAR) == anno) {
                    	if(auxSesion!=null)
...
/*        if(targetIdList!= null && !targetIdList.isEmpty()) {
            List<Object> maxVersions = documentoDAO.getLastVersionListMaxVersion(targetIdList);
            Map<Long, Integer> mapVersions = new HashMap<>();
            for (Object o : maxVersions) {
                Object[] l = (Object[]) o;
                Long id = (Long) l[0];
                Integer version = (Integer) l[1];
                documentoVOs.add(documentoDAO.obtenerDocumentoByParentMaxVersion(id, version).toVO());
            }
        }*/
        //Collections.sort(documentoVOs, new sortbyFechaCarga());
    ...
    public DocumentoVO obtenerDocumentoByDocumentId(Long documentId){
        DocumentoBO documentoBO = documentoDAO.obtenerDocumentoByDocumentId(true, documentId);
    ...
    public List<DocumentoVO> obtenerDocumentosRecientes(UserVO user){
        List<Integer> targetIdList = documentoDAO.getDocumentIdListByLastVersion();
        List<DocumentoVO> returnedListedDocument = new ArrayList<>();
        if (targetIdList != null && !targetIdList.isEmpty()) {
            List<DocumentoBO> documentoBOs = documentoDAO.obtenerDocumentosRecientes(true, 10, user.getId(), targetIdList);
            List<DocumentoVO> documentoVOs = new ArrayList<>();
...
/*    public List<DocumentoVO> obtenerDocumentosRecientes(UserVO user){
    ...
    /**
     * @see cl.bice.gestionactas.ejb.svc.GestorDocumentalSvc#existDocument(cl.bice.gestionactas.ejb.vo.DocumentoVO, cl.bice.gestionactas.ejb.vo.DocumentoVO )
     */
    @Override
    ...
    /**
     * Permite guardar un documentoBO en la base de datos tentiendo en cuenta los conceptos de bloqueo
     * @param documentoVO Documento que viene desde la capa de presentacion
     * @param documentId id del docuemnto padre
     * @param originalBaseName Nombre del archivo original subido en la capa de presentacion
     * @param originalExtension Extension del archivo original.
     */
    private void guardaNuevoDocumentoBO(DocumentoVO documentoVO, String documentId, String originalBaseName, String originalExtension) {
        DocumentoBO documentoBO = new DocumentoBO(documentoVO.getTipoCorporacion(), documentoVO.getTipoDocumento(), null);
    ...
    public String obtenerToken(String userName, String fileName, byte[] fileStream, DocumentoVO documentoVO, String originalDocumentName, String originalDocumentExtension) {
        SaveDocument saveDocument = new SaveDocumentIDC();
        String documentId = saveDocument.obtenerToken(userName, fileName, fileStream, documentoVO, originalDocumentName, originalDocumentExtension);
        System.out.println("DOCUMENTO ---------------------------------------- "+documentoVO.toString());
        guardaNuevoDocumentoBO(documentoVO, documentId, originalDocumentName, originalDocumentExtension);
        return documentId;
    }
    ...
    public String eliminarDocumento(DocumentoVO documentoVO) {
        String documentId="";
        try {
        	List<Long> lista = documentoDAO.getDocumentIdListByParent(documentoVO.getParentDocument().getId());
        	for(int i=0; i<lista.size();i++){
	            logger.info("ID doc: "+lista.get(i));
		        documentoDAO.lock(lista.get(i));
		        DocumentoBO documentoBO = documentoDAO.getForUpdate(lista.get(i));
		        documentId = documentoBO.getTokenDocumento();
		        documentoBO.setEstado(false);
		        //documentId = documentoDAO.updateDocumentFlgEstado(lista.get(i));
		        documentoDAO.update(documentoBO);
        	}
        }catch(Exception e){
        	e.printStackTrace();
  ...
  //03/01/2017
    public List<BigDecimal> filtrarDocumentosID(String titulo, String descripcion, Long tipoDocumento, Long tipoCorporacion, Long numSesion, Integer year,
    		Long userID, int first, int max){
    	...
    	String queryBuild = new String("SELECT DOC_ROW.doc_id FROM (SELECT docum.*, rownum rn FROM (" +
    	...
    	if(year!= null)
			  queryBuild = queryBuild+" AND EXTRACT(YEAR FROM d.fec_fechacreacion)= :year ";
    	if(numSesion!= null)
			  queryBuild = queryBuild+" AND d.num_sesion= :numSesion ";
		if(tipoDocumento!= null)
			  queryBuild = queryBuild+" AND d.flg_tipodocumento_id= :tipoDocumento ";
		if(tipoCorporacion!= null)
			  queryBuild = queryBuild+" AND d.doc_tipocorporacion_id= :tipoCorporacion ";
    ...
    public Integer countFiltrarDocumentosSesionID(String titulo, String descripcion, Long tipoDocumento, Long tipoCorporacion, Long numSesion, Integer year,
    		Long userID){
    	EntityManager entityManager = ServiceContext.current().getEM();
    	...
    	String queryBuild = new String("SELECT * "+
    			...
    			"    ) ");
    	if(year!= null)
    		queryBuild = queryBuild+" AND extract(YEAR FROM doc.fec_fechacreacion)= :year ";
    	if(tipoDocumento!= null)	
    		queryBuild = queryBuild+" AND doc.flg_tipodocumento_id = :tipoDocumento ";
    	if(tipoCorporacion!= null)	
			queryBuild = queryBuild+" AND doc.doc_tipocorporacion_id= :tipoCorporacion ";
    	if(numSesion!= null)
